...
def search_single_day():
    ...
    prices = df.groupby(['date', 'airlines'])['price'].min().reset_index().pivot_table(index = 'date', columns = 'airlines', values = 'price').to_json(orient='records')

    per_airline = df.groupby('airlines')[['price','fare_basis','fare_classes']].last().reset_index().to_json(orient='records')
